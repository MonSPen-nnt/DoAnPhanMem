...
namespace BanTrangSuc.Controllers
{
    public class UserController : Controller
    {
            ...
            if (donHang.TrangThai == "Đang giao")
            {
                ...
                foreach (var chiTiet in chiTietDonHangList)
                {
                    ...
                    var baoHanh = new BaoHanh
                    {
                        ...
                        MoTa = "Bảo hành cho sản phẩm " + chiTiet.MaSanPham,
        ...
        //{
        //    // Tìm kiếm sản phẩm tương ứng với mã sản phẩm
        //    var baoHanh = new BaoHanh
        //    {
        //        MaSanPham = maSanPham,
        //        MaNguoiDung = int.Parse(HttpContext.Session.GetString("UserID")),
        //        NgayBaoHanh = DateTime.Now,
        //        NgayKetThuc = DateTime.Now.AddMonths(1), // Ngày kết thúc bảo hành là 1 tháng sau
        //        MoTa = "Yêu cầu bảo hành sản phẩm",
        //        TrangThai = false // Mặc định là false cho đến khi xác nhận
        //    };

        //    // Thêm thông tin bảo hành vào cơ sở dữ liệu
        //    db.BaoHanhs.Add(baoHanh);
        ...
        //    return RedirectToAction("OrderHistory"); // Chuyển hướng đến trang lịch sử đơn hàng
        //}
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult YeuCauBaoHanh(int maSanPham)
        {
            var donHang = db.DonHangs
                .Include(dh => dh.ChiTietDonHangs)
                .FirstOrDefault(dh => dh.ChiTietDonHangs.Any(ct => ct.MaSanPham == maSanPham && dh.TrangThai == "Đã nhận"));
            ...
            // Tiến hành xử lý yêu cầu bảo hành
            var baoHanh = new BaoHanh
            {
                MaSanPham = maSanPham,
                MaNguoiDung = int.Parse(HttpContext.Session.GetString("UserID")),// Lấy ID người dùng từ cookie (nếu đã đăng nhập)
                NgayBaoHanh = DateTime.Now,
                NgayKetThuc = DateTime.Now.AddMonths(1), // Ngày kết thúc bảo hành là 1 tháng sau
                MoTa = "Yêu cầu bảo hành sản phẩm",
                TrangThai = false // Mặc định là false cho đến khi xác nhận
